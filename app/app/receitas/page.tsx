...
const FORM_VAZIO = { descricao: '', valor: '', categoria: 'Outros', data_recebimento: '', tipo: 'unica', parcelas: '1', observacao: '' }
...
export default function ReceitasPage() {
  ...
  const salvar = async () => {
    ...
    if (editandoId) {
      const cOriginal = receitas.find(c => c.id === editandoId)
      ...
      let baseDate = payload.data_recebimento ? new Date(payload.data_recebimento + 'T12:00:00Z') : new Date()
      ...
      for (let i = 0; i < qtd; i++) {
        ...
        inserts.push({
          ...payload,
          data_recebimento: currentVencimento.toISOString().split('T')[0],
  ...
  const executarEdicaoGrupo = async (applyToFuture: boolean) => {
    ...
    if (applyToFuture) {
      ...
      }).eq('grupo_id', original.grupo_id).eq('status', 'pendente').gte('data_recebimento', original.data_recebimento)
      
      if (payload.data_recebimento !== original.data_recebimento) {
        await supabase.from('receitas').update({ data_recebimento: payload.data_recebimento }).eq('id', original.id)
      }
      
      if (error) { showToast('Erro ao atualizar em lote.', 'error') }
      else { showToast('Parcelas atualizadas ✓') }
    } else {
      const { error } = await supabase.from('receitas').update(payload).eq('id', original.id)
  ...
  const marcarRecebido = async (id: string) => {
    await supabase.from('receitas').update({ status: 'recebido' }).eq('id', id)
  ...
  const statusIcon = (c: any) => {
    if (c.status === 'recebido') return <Check size={14} color="var(--green-400)" />
    if (c.data_recebimento && c.data_recebimento < hoje) return <AlertCircle size={14} color="var(--red)" />
    return <Clock size={14} color="var(--amber)" />
  }

  const pendentesRaw = receitas.filter(c => c.status !== 'recebido')
  const pagasRaw = receitas.filter(c => c.status === 'recebido')
  ...
  for (const c of pendentesRaw) {
    if (!c.grupo_id) {
      ...
      if (!gruposMap.has(c.grupo_id)) {
        ...
        if (new Date(c.data_recebimento) < new Date(existente.data_recebimento)) {
          gruposMap.set(c.grupo_id, c)
        }
      }
    }
  ...
  pendentes.sort((a, b) => new Date(a.data_recebimento).getTime() - new Date(b.data_recebimento).getTime())

  // Para as recebidas, filtramos pelo mês selecionado se estiver na aba histórico
  const pagas = pagasRaw.filter(c => {
    if (!c.data_recebimento) return false
    const d = new Date(c.data_recebimento + 'T00:00:00')
  ...
  return (
    <div style={{ padding: '20px 16px' }}>
        ...
        <div style={{
          ...
          <div className="card anim-up" style={{ padding: '24px', width: '100%', maxWidth: 480 }}>
            ...
            <p style={{ fontSize: 13, color: 'var(--text-2)', marginBottom: 20 }}>
              Esta receita faz parte de um parcelamento/recorrência. Deseja aplicar as alterações de valor e descrição em todas as parcelas futuras também?
            </p>
        ...
        <div style={{
          ...
          <div className="card anim-up" style={{ padding: '24px', width: '100%', maxWidth: 400 }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 20 }}>
              <h3 style={{ fontSize: 18, fontWeight: 500, color: 'var(--text-1)' }}>Detalhes da Receita</h3>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                <div>
                  ...
                  <div style={{ fontSize: 16, fontFamily: 'var(--font-display)', color: detalhesModal.data_recebimento < hoje ? 'var(--red)' : 'var(--text-1)' }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 15, color: 'var(--text-1)' }}>{detalhesModal.data_recebimento ? new Date(detalhesModal.data_recebimento + 'T00:00:00').toLocaleDateString('pt-BR') : '-'}</div>
                </div>
              </div>
      ...
      {confirmDeleteId && (() => {
        const cToDelete = receitas.find(c => c.id === confirmDeleteId)
        ...
        return (
          <div style={{
            ...
            <div className="card anim-up" style={{ padding: '24px', width: '100%', maxWidth: 480 }}>
              <h3 style={{ fontSize: 16, fontWeight: 500, marginBottom: 8 }}>Excluir receita?</h3>
              <p style={{ fontSize: 13, color: 'var(--text-2)', marginBottom: 20 }}>
                {isGroup ? 'Esta receita faz parte de um parcelamento/recorrência. Deseja excluir apenas ela ou todas as parcelas pendentes futuras?' : 'Essa ação não pode ser desfeita.'}
              </p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {isGroup && (
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        <div>
          <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 24, marginBottom: 2 }}>Receitas</h1>
          <p style={{ fontSize: 13, color: 'var(--text-2)', fontWeight: 300 }}>{abaAtiva === 'pendentes' ? `${pendentes.length} pendentes` : 'Histórico de recebimentos'}</p>
        </div>
      ...
      {showForm && (
        <div className="card anim-up" style={{ padding: '20px', marginBottom: 20, borderColor: 'var(--border-em)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16 }}>
            <h3 style={{ fontSize: 15, fontWeight: 500 }}>{editandoId ? 'Editar receita' : 'Nova receita'}</h3>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              ...
              <div>
                <label style={{ fontSize: 11, color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4, display: 'block' }}>Recebimento</label>
                <input className="input" type="date" value={form.data_recebimento} onChange={e => setForm({ ...form, data_recebimento: e.target.value })} />
              </div>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              <div>
            ...
            <button onClick={salvar} className="btn-primary" disabled={saving || !form.descricao || !form.valor || (form.tipo === 'parcelada' && (!form.parcelas || Number(form.parcelas) < 2))} style={{ width: '100%', marginTop: 8 }}>
              {saving ? 'Salvando...' : editandoId ? 'Salvar alterações' : 'Criar receita'}
            </button>
          </div>
        </div>
      )}
      ...
      {abaAtiva === 'historico' && (
        <div style={{ marginBottom: 20 }}>
          ...
          {pagas.length > 0 && (
            <div className="card" style={{ padding: '16px', background: 'var(--green-dim)', border: 'none', marginBottom: 16 }}>
              <div style={{ fontSize: 11, color: 'var(--green-600)', textTransform: 'uppercase', marginBottom: 4 }}>Total recebido no mês</div>
              <div style={{ fontSize: 20, fontWeight: 600, color: 'var(--green-600)' }}>{fmt(totalHistorico)}</div>
            </div>
          )}
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="card" style={{ padding: '40px 20px', textAlign: 'center' }}>
          ...
          <p style={{ color: 'var(--text-2)', fontSize: 14 }}>
            {abaAtiva === 'pendentes' ? 'Nenhuma receita pendente.' : 'Nenhuma receita recebida neste mês.'}
          </p>
        </div>
      ) : (
        <>
                  ...
                  <div key={c.id} className="card" style={{ padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer', transition: 'background 0.2s' }} onClick={() => setDetalhesModal(c)}>
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: 11, color: 'var(--text-3)' }}>
                        {c.categoria} {c.data_recebimento && `· recebimento ${new Date(c.data_recebimento + 'T00:00:00').toLocaleDateString('pt-BR')}`}
                      </div>
                    </div>
                    <div style={{ textAlign: 'right' }}>
                      <div style={{ fontSize: 15, fontWeight: 500, color: c.data_recebimento < hoje ? 'var(--red)' : 'var(--text-1)' }}>
                        {fmt(Number(c.valor))}
                      </div>
                      <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 4 }}>
                        <button onClick={(e) => { e.stopPropagation(); marcarRecebido(c.id) }} style={{
                ...
                {pagas.map(c => (
                  <div key={c.id} className="card" style={{ padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 12, opacity: 0.8, cursor: 'pointer' }} onClick={() => setDetalhesModal(c)}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: 11, color: 'var(--text-3)' }}>{c.categoria} · recebido em {c.data_recebimento ? new Date(c.data_recebimento + 'T00:00:00').toLocaleDateString('pt-BR') : '-'}</div>
                    </div>
                    <div style={{ textAlign: 'right' }}>
